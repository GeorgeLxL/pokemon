...
export default function Cards({ cards }: Props) {
  ...
  const cardCounts = (card: CardRow) => {
    ...
    const countElements = newArr.map((count, idx) => (
      <tr
        key={idx}
        className={`w-full text-md ${idx%2===0&&'bg-gray-200'}`}
        >
      ...
      {categories.map(({ id, name_var }) => {
        ...
        return (
          <div
            ...
            onClick={() => toggleCategory(Number(id))} style={{ cursor: "pointer" }}
          >
            <div
            ...
            {!isCollapsed && (
              <div className="space-y-2 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4" onClick={e => e.stopPropagation()}>
